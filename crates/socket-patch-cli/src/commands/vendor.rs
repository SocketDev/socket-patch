...
pub(crate) async fn persist_vendor_entry(
    ...
    if let Some(prev) = &prev {
        ...
        // Same invariant for the cargo lock originals: the removed
        // `source`/`checksum` pair is NOT recoverable offline — the ledger
        // entry is its only home. A re-vendor over already-detached wiring
        // records `lock: None` (there was nothing left to detach), so
        // inserting the fresh entry verbatim would silently destroy the
        // first run's originals and a later `--revert` could only shrug.
        // Carry them forward from the entry being replaced.
