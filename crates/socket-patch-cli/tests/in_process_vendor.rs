...
// ─────────────────────────────────────────────────────────────────────
// 8e. yarn berry on the Windows file shapes: CRLF, BOM, mixed endings
// ─────────────────────────────────────────────────────────────────────

/// The berry root manifest the way yarn pretty-prints it (LF form).
const BERRY_WIN_PKG: &str = r#"{
  "name": "berry-win",
  "version": "1.0.0",
  "private": true,
  "dependencies": {
...
/// The matching yarn 4 lock (LF form).
fn berry_win_lock() -> String {
    format!(
        "# This file is generated by running \"yarn install\" inside your project.\n\
         # Manual changes might be lost - proceed with caution!\n\n\
         __metadata:\n  version: 8\n  cacheKey: 10c0\n\n\
...
/// `text` in the Windows shape yarn berry writes (`os.EOL` = CRLF for a
/// file it creates or first pretty-prints), optionally BOM'd by an editor.
fn windows_shape(text: &str, bom: bool) -> String {
    format!(
        "{}{}",
        if bom { "\u{feff}" } else { "" },
        text.replace('\n', "\r\n")
...
/// A berry project (root manifest + lock as given, `.yarnrc.yml`, the
/// installed copy) plus the offline manifest + blob `vendor` reads.
fn stage_berry_project(root: &Path, pkg: &str, lock: &str) {
...
/// yarn berry writes BOTH files CRLF on Windows (a new lock and the
/// manifest it first pretty-prints get `os.EOL`). Vendoring must keep each
/// file's layout — CRLF, BOM, trailing newline — so the wired pair is what
/// yarn itself would have written; the re-run is byte-stable; and
/// `vendor --revert` restores both files byte-for-byte (the Windows CI
/// failure this pins: the revert wrote package.json back LF).
#[tokio::test]
async fn berry_crlf_and_bom_vendor_and_revert_round_trip_byte_exact() {
    for bom in [false, true] {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        let (pkg, lock) = (
            windows_shape(BERRY_WIN_PKG, bom),
            windows_shape(&berry_win_lock(), bom),
        );
        stage_berry_project(root, &pkg, &lock);

        let (code, env) = vendor_cli(root, &[]);
        assert_eq!(code, 0, "bom={bom}: vendor: {env:#}");
        assert_eq!(env["summary"]["applied"], 1, "bom={bom}: {env:#}");
        for rel in ["package.json", "yarn.lock"] {
            let text = std::fs::read_to_string(root.join(rel)).unwrap();
            assert_eq!(
                text.matches('\n').count(),
                text.matches("\r\n").count(),
                "bom={bom}: {rel} keeps CRLF: {text:?}"
            );
            assert_eq!(text.starts_with('\u{feff}'), bom, "bom={bom}: {rel} BOM");
            assert!(
                text.contains(".socket/vendor/npm/"),
                "bom={bom}: {rel} wired"
            );
        }
        let wired: Vec<Vec<u8>> = ["package.json", "yarn.lock"]
            .iter()
            .map(|rel| std::fs::read(root.join(rel)).unwrap())
            .collect();

        let (code, env) = vendor_cli(root, &[]);
        assert_eq!(code, 0, "bom={bom}: re-vendor: {env:#}");
        for (rel, bytes) in ["package.json", "yarn.lock"].iter().zip(&wired) {
            assert_eq!(
                &std::fs::read(root.join(rel)).unwrap(),
                bytes,
                "bom={bom}: re-vendor leaves {rel} byte-identical"
            );
        }

        let (code, env) = vendor_cli(root, &["--revert"]);
        assert_eq!(code, 0, "bom={bom}: revert: {env:#}");
        assert_eq!(
            std::fs::read_to_string(root.join("package.json")).unwrap(),
            pkg,
            "bom={bom}: package.json restored byte-for-byte"
        );
        assert_eq!(
            std::fs::read_to_string(root.join("yarn.lock")).unwrap(),
            lock,
            "bom={bom}: yarn.lock restored byte-for-byte"
        );
        assert!(!root.join(".socket/vendor").exists(), "bom={bom}: pruned");
    }
}

/// A berry lock or root manifest whose line endings MIX CRLF and LF has no
/// single style to keep (and yarn itself rejects such a lock under
/// `--immutable`): the package fails with the dedicated code before any
/// write, both files byte-identical and no vendor tree created.
#[tokio::test]
async fn berry_mixed_line_endings_fail_closed_with_code() {
    let crlf_lock = windows_shape(&berry_win_lock(), false);
    let crlf_pkg = windows_shape(BERRY_WIN_PKG, false);
    let mixed = |crlf: &str| crlf.replacen("\r\n", "\n", 1);
    for (label, pkg, lock) in [
        ("mixed lock", crlf_pkg.clone(), mixed(&crlf_lock)),
        ("mixed package.json", mixed(&crlf_pkg), crlf_lock.clone()),
    ] {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        stage_berry_project(root, &pkg, &lock);
        let (code, env) = vendor_cli(root, &[]);
        assert_eq!(code, 1, "{label}: {env:#}");
        let failed = find_event(&env, "failed", Some("vendor_yarn_berry_mixed_line_endings"));
        assert!(
            failed.to_string().contains("yarn install"),
            "{label}: the remedy is named: {failed}"
        );
        assert_eq!(
            std::fs::read_to_string(root.join("package.json")).unwrap(),
            pkg,
        ...
        assert!(!root.join(".socket/vendor").exists(), "{label}");
    }
}
...
async fn mount_berry_hosted_api(server: &wiremock::MockServer) -> String {
    ...
    let hosted_url = format!(
        "{}/patch/npm/left-pad/1.3.0/44444444-4444-4444-8444-444444444444/{UUID}/left-pad-1.3.0.tgz",
        server.uri()
    );
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "packages": [{ "purl": PURL, "patches": [{
                ...
                "severity": "high", "title": "berry takeover fixture"
            }]}],
...
/// `scan --mode hosted --json --yes` through the binary.
fn hosted_scan_cli(root: &Path, api_url: &str) -> (i32, Value) {
...
async fn berry_crlf_takeovers_round_trip_both_directions() {
    ...
    let assert_crlf = |root: &Path, ctx: &str| {
        for rel in ["package.json", "yarn.lock"] {
            let text = std::fs::read_to_string(root.join(rel)).unwrap();
            assert_eq!(
                text.matches('\n').count(),
    ...
    assert!(
        !vendored_lock.contains("__archiveUrl") && vendored_lock.contains(".socket/vendor/npm/"),
        "fully vendored: {vendored_lock:?}"
    );
...
async fn revert_after_drift_keeps_artifact_and_ledger_then_completes() {
    ...
    // ownership-gate outcome as a hosted patch.socket.dev overlay). The
    // drifted value must be neither ours nor the recorded pre-vendor
    // original — a lock already back at the original is CONVERGED, and the
    // revert completes instead of keeping (see
    // `revert_completes_when_lock_already_matches_the_original`).
    let mut drifted: Value = serde_json::from_slice(&vendored_lock).unwrap();
...
async fn reconcile_keeps_drifted_entry() {
    ...
    // Drop the patch from the manifest AND drift the lock (to a value that
    // is neither ours nor the recorded pre-vendor original — genuine
    // third-party drift).
    std::fs::write(fx.manifest_path(), b"{\"patches\": {}}\n").unwrap();
...
/// KEEP-GATE LIVENESS: a lock whose entry already equals the recorded
/// pre-vendor original is CONVERGED, not drifted — the user (or an earlier
/// partial revert) already restored it, e.g. via `git checkout`. The revert
/// must complete fully — artifacts removed, ledger pruned, counted
/// `Removed` — never keep the entry with "undo the drift" advice that can
/// never be satisfied.
#[tokio::test]
async fn revert_completes_when_lock_already_matches_the_original() {
    let fx = npm_fixture();
    assert_eq!(vendor_run(vendor_args(fx.root())).await, 0);

    // The user restored the pre-vendor lock by hand (git checkout).
    ...
    assert_eq!(
        env["summary"]["removed"], 1,
        "the converged revert completes: {env:#}"
    );
    assert_eq!(
        env["summary"]["skipped"], 0,
        "nothing was kept, so nothing may be counted skipped: {env:#}"
    );
    assert!(
        events(&env)
            .iter()
            .all(|e| e["errorCode"] != "vendor_revert_kept"
                && e["errorCode"] != "vendor_lock_entry_drifted"),
        "an already-restored entry is converged, not drifted: {env:#}"
    );
    assert_eq!(fx.lock_bytes(), fx.original_lock, "lock stays restored");
    assert!(
        !fx.vendor_dir().exists(),
        ".socket/vendor fully pruned once the revert converges"
    );
    let state_gone = !fx.state_path().exists()
        || serde_json::from_slice::<Value>(&std::fs::read(fx.state_path()).unwrap())
