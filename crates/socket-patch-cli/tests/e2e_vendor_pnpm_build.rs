...
//!
//! `pnpm_get_uuid_vendored_fresh_checkout_frozen_offline` is the get-driven
...
async fn pnpm_vendor_fresh_checkout_frozen_offline_install_and_revert() {
    ...
    run_pnpm_capstone(PNPM_PRIMARY, VendorDriver::VendorCli).await;
    ...
    if has_corepack_pm(PNPM_SECONDARY) {
        ...
        run_pnpm_capstone(PNPM_SECONDARY, VendorDriver::VendorCli).await;
    } else {
    ...
    if has_corepack_pm(PNPM_TERTIARY) {
        ...
        run_pnpm_capstone(PNPM_TERTIARY, VendorDriver::VendorCli).await;
    } else {
...
/// `--frozen-lockfile --offline` proof. The record arrives over a mocked API
...
#[tokio::test(flavor = "multi_thread")]
async fn pnpm_get_uuid_vendored_fresh_checkout_frozen_offline() {
    if !has_corepack_pm(PNPM_PRIMARY) {
        println!(
            "SKIP e2e_vendor_pnpm_build (get twin): `corepack {PNPM_PRIMARY}` unavailable \
             (corepack not installed or pnpm not fetchable)"
        );
        return;
    }
    run_pnpm_capstone(PNPM_PRIMARY, VendorDriver::GetUuid).await;
}

async fn run_pnpm_capstone(pm: &str, driver: VendorDriver) {
    ...
    if driver == VendorDriver::GetUuid {
        ...
        // `vendor` front door's own contract, pinned by the VendorCli legs;
