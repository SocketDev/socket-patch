...
/// (name + version kept, source + checksum dropped).
...
/// `version` key, v3 — is the same live `[patch]`; a v1–v4 lock that
/// re-resolved the crate from crates.io is not.
#[test]
fn cargo_vendored_a_attests_in_every_lock_version() {
    ...
    for version in 1..=4u8 {
        ...
        fx.put("Cargo.lock", cargo_lock(version, ""));
        let run = fx.vex(&["--proxy-url", &api.uri()]);
        assert_attested(
            &fx,
            &run,
