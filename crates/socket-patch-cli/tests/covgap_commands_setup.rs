...
fn gem_check_bundler_1x_lock_wired_points_at_remove() {
    ...
    assert!(
        err.contains("1.17.3"),
        "must name the detected version: {err}"
    );
...
fn exclude_already_persisted_skips_manifest_rewrite() {
    ...
    let (code, v) = run_json(
        cwd,
        &["setup", "--yes", "--json", "--exclude", "packages/a"],
    );
    assert_eq!(code, 0, "{v}");
    assert_eq!(v["status"], "success", "{v}");
...
fn setup_exclude_in_empty_dir_writes_no_socket_dir() {
    ...
    let (code, v) = run_json(
        cwd,
        &["setup", "--yes", "--json", "--exclude", "packages/x"],
    );
    assert_eq!(code, 0, "{v}");
    assert_eq!(v["status"], "no_files", "{v}");
    assert!(
...
fn setup_dry_run_exclude_persists_nothing() {
    ...
    let (code, v) = run_json(
        cwd,
        &["setup", "--dry-run", "--json", "--exclude", "packages/b"],
    );
    assert_eq!(code, 0, "{v}");
...
fn setup_exclude_write_failure_surfaces_persist_warning() {
    ...
    assert_eq!(
        code, 0,
        "the hooks were written; only persistence was skipped: {v}"
    );
    assert_eq!(v["status"], "success", "{v}");
    assert!(
...
fn check_python_states_without_hook_and_missing_requirements() {
    ...
    assert!(
        pth["error"].is_null(),
        "NotFound-for-Requirements is not an error: {v}"
    );
}
...
fn remove_json_surfaces_poetry_lock_refresh_warning() {
    ...
    assert!(
        warnings.iter().any(|w| w
            .as_str()
            .is_some_and(|w| w.contains("could not run `poetry"))),
