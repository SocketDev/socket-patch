...
* **rollback reverts vendored and hosted state by default (v5.0, MAJOR — was: excluded)**: the
  ...
  skip); acted-on entries land in the new `vendoredReverted`/`vendoredPreserved`/`vendoredKept`
...
| `vendor_revert_kept`      | `skipped` + top-level error | remove (v5.0): the vendored revert drift-kept (`kept_artifact`), so the ledger entry AND the manifest entry were both kept. When EVERY matching entry drift-kept, the run exits 1 with `status: partialFailure` and this top-level code (`summary.removed` stays 0) — the remove did not happen, but the identifier DID match (never `not_found`). Remedy: re-run `scan --mode vendored` to normalize, then remove. Rollback's counterpart is the `vendoredKept: []` envelope array (also exit 1). |
