...
| `cargo_lock_untaggable` | `failed` | vendor / scan / get `--mode vendored` (cargo, v5.0): the Cargo.lock entry cannot carry the copy's tagged version consistently (a dependency reference in a spelling the edit does not own, a v1 `replace` naming the crate, or an entry already at the tagged version). Refused before any write. |
| `cargo_copy_untaggable` | `failed` (error prefix) | vendor / scan / get `--mode vendored` (cargo, v5.0): the copy's `Cargo.toml` has no literal `[package] version` string that can be rewritten byte-exactly (or it names another version); nothing is swapped in. |
