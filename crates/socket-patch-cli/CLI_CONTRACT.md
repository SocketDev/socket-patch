...
5. **In-repo and committable.** `setup` writes only inside the working tree: `package.json`,
   ...
   `Warning: unreadable vendor state (…)` line, never as a `configured` verdict. It never writes outside
...
**Confirmation prompt.** A wet, non-preserve run with work prompts once, remove-style, composing only the clauses that apply: `[Roll back N patch(es) and remove them from the local manifest][, and delete M vendored artifact(s) and their ledger records][, and unwind H hosted redirect(s)]?` (clauses joined by `, and `, first letter capitalized; a hosted ledger with leftover edits but no records gets `replay K leftover hosted redirect edit(s)` instead of the unwind clause) — default yes, auto-accepted under `--yes`/`--json`/non-TTY (the shared `confirm` semantics; CI unaffected). Decline prints `Rollback cancelled.` and exits 0. `--dry-run` and `--preserve-state` runs are prompt-free (they delete no local state).
