...
| `--update --dry-run` | **Check-only**: one metadata request, zero downloads, zero mutation, exit 0 either way. `--json` details carry `{current, latest, updateAvailable, target, asset, path}` — the cheap scriptable "is an update available" probe. |
