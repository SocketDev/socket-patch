...
**Gem stale-install guard (additive warning)**: the gem hosted rewrite is pure Gemfile/lock text, so a gem ALREADY materialized under the project's bundle paths keeps its upstream bytes — the next `bundle install` prints `Using <gem>` and never refetches, on **every** bundler major (live-verified 2026-08-19 on 1.17.3 / 2.7.2 / 4.0.18: bundler 4's CHECKSUMS verify at download time only, and nothing is downloaded; `bundle install --force`/`--redownload` re-install from the stale cached `.gem` instead of re-fetching — bundler 1 silently, bundler 4 with an exit-37 checksum refusal that still leaves the upstream bytes installed). After the rewrite, a hosted run therefore probes the installed-gem discovery paths (the same ruby-crawler discovery `apply` uses) for each confirmed gem redirect and hash-compares the materialization against the patch record's `afterHash` file map: a materialization with every file at `afterHash` is already patched and never warns (an agent→hosted migration stays quiet by construction); anything else emits `redirect_gem_stale_install` (JSON `redirect.warnings[]` + a code-tagged stderr line) naming the installed dir, the cache `.gem`, and the `specifications` entry, and prescribing the **verified** remedy — remove those three paths, then `bundle install` (never `--force`/`--redownload`, which the detail calls out as non-remedies). The probe is read-only (nothing is deleted), skipped on `--dry-run` (nothing was rewritten), silent when the patch record could not be fetched (`record_fetch_failed` already fired; no `afterHash` map, no sound judgment), and re-fires on idempotent re-scans until the stale materialization is gone. Exit code and `status` are unchanged (warning-only, the hosted-refusal posture).
...
| `vendored`                | `skipped`        | apply (every ecosystem) + scan `--apply`: the package is managed by `socket-patch vendor`; the command yields ownership (scan also skips the download). Rollback surfaces the same skip via its `vendored: []` array. Scan `--apply --json` additionally surfaces one run-level `vendored_ownership_retained` warning naming the skipped purls (additive; exit/status unchanged). |
...
| `redirect_gem_stale_install` | `redirect.warnings[]` (warning) | scan `--mode hosted` (gem): a confirmed gem redirect has a stale UNPATCHED materialization under the project's bundle paths — `bundle install` reuses it and never refetches on every bundler major, and `--force`/`--redownload` re-install from the stale cached `.gem` (all live-verified on bundler 1.17.3/2.7.2/4.0.18). The detail names the installed dir + cache `.gem` + `specifications` entry and prescribes removing them, then `bundle install`. Detection hash-compares the materialization against the patch record's `afterHash` map, so an already-patched install (e.g. agent-mode apply) never warns. See "Gem stale-install guard". |
