...
fn pre_1_4_writer(lock_text: &str) -> bool {
    let Ok(lock) = lock_text.parse::<DocumentMut>() else {
        return false;
    };
    match lock_version(&lock) {
        Ok("0" | "1.0" | "1.1") => true,
        Ok("2.0") => !matches!(generated_by_version(lock_text), Some(v) if v >= (1, 4)),
        _ => false,
    }
}
        ...
        for &(dep, sha256) in &usable {
            ...
            match rewrite_poetry_lock(
                ...
                Ok(Some(rewritten)) if rewritten != content => {
                    ...
                    content = rewritten;
                    if !stale_warned && pre_1_4_writer(&content) {
                        stale_warned = true;
                        result.warnings.push(RewriteWarning {
                            code: "redirect_poetry_stale_install_risk".into(),
                            detail: format!(
                                "{path} was written by Poetry < 1.4, which does not replace an \
                                 already-installed package at the same version: an existing \
                                 virtualenv keeps the upstream {} until it is recreated (or the \
                                 package is `pip uninstall`ed) before `poetry install`; fresh \
                                 installs pick up the patched wheel",
                                dep.name
                            ),
                        });
                    }
