...
use super::npm_common::{
    done_failure, guard_coordinates, refused, stage_patch_pack, tgz_rel_leaf,
};
...
pub async fn vendor_yarn_berry(
    ...
    if let Some(res) = pkg_obj.get("resolutions") {
        ...
        for (selector, value) in res_obj {
            let sel_name = split_pattern(selector).map(|(n, _)| n).unwrap_or(selector.as_str());
            if sel_name != name {
                continue;
            }
    ...
    if let Err(e) =
        commit_pair(project_root, &new_pkg_bytes, &pkg_bytes, new_lock_text.as_bytes()).await
    {
    ...
    let wiring = vec![
        WiringRecord {
            ...
            action: if existing_entry { WiringAction::Rewritten } else { WiringAction::Added },
        ...
        WiringRecord {
            ...
            key: Some(lock_key),
            original: if target.is_ours { None } else { Some(lines_to_json(&target.lines)) },
...
async fn commit_pair(
    ...
    if let Err(e) = atomic_write_bytes(&project_root.join(YARN_LOCK), new_lock).await {
        return match atomic_write_bytes(&pkg_path, orig_pkg).await {
            Ok(()) => Err(format!("cannot write {YARN_LOCK}: {e} ({PACKAGE_JSON} restored)")),
            Err(e2) => Err(format!(
...
fn scan_berry_target(
    ...
    match found.len() {
        ...
        _ => Err((
            "vendor_override_conflict",
            format!("multiple yarn.lock entries resolve {name}@{version}; refusing the \
                     ambiguous rewrite"),
        )),
    }
}
...
fn carried_sections(lines: &[String]) -> Vec<String> {
    const OWNED: [&str; 5] = ["version", "resolution", "checksum", "languageName", "linkType"];
    let mut out = Vec::new();
    let mut i = 1;
    while i < lines.len() {
...
"#;
    ...
    impl Fixture {
        ...
        async fn assert_untouched(&self) {
            assert_eq!(tokio::fs::read(self.pkg_path()).await.unwrap(), self.pkg_bytes);
            assert_eq!(tokio::fs::read(self.lock_path()).await.unwrap(), self.lock_bytes);
    ...
    async fn fixture_with(pkg: &str, lock: &str) -> Fixture {
        ...
        tokio::fs::write(root.join(PACKAGE_JSON), pkg.as_bytes()).await.unwrap();
        tokio::fs::write(root.join(YARN_LOCK), lock.as_bytes()).await.unwrap();
        tokio::fs::write(root.join(YARNRC), YARNRC_DEFAULT).await.unwrap();
    ...
    async fn b3_fixture_oracle_pair_edit_is_byte_exact() {
        ...
        assert_eq!((pkg_rec.file.as_str(), pkg_rec.kind.as_str()), (PACKAGE_JSON, KIND_RESOLUTION));
        ...
        assert_eq!((lock_rec.file.as_str(), lock_rec.kind.as_str()), (YARN_LOCK, KIND_LOCK_ENTRY));
    ...
    async fn non_10c0_cache_key_is_refused_before_any_write() {
        ...
        let detail = expect_refused(fx.vendor(false).await, "vendor_yarn_berry_cache_unsupported");
        assert!(detail.contains("`10`"), "names the found cacheKey: {detail}");
    ...
    async fn checksum_changing_yarnrc_knob_is_refused_by_name() {
        ...
        .unwrap();
        let detail = expect_refused(fx.vendor(false).await, "vendor_yarn_berry_cache_unsupported");
        assert!(detail.contains("compressionLevel"), "names the knob: {detail}");
        fx.assert_untouched().await;
    ...
    async fn missing_entry_and_other_version_guards() {
        ...
        let fx = fixture_with(B3_BEFORE_PKG, &lock).await;
        let detail = expect_refused(fx.vendor(false).await, "vendor_override_conflict");
        assert!(detail.contains("1.2.0"), "names the other version: {detail}");
        fx.assert_untouched().await;
    }
    ...
    async fn commit_pair_unwinds_package_json_when_the_lock_write_fails() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        tokio::fs::write(root.join(PACKAGE_JSON), b"orig-pkg").await.unwrap();
    ...
    fn helper_grammar() {
        ...
        assert_eq!(root_workspace_name(&blocks).as_deref(), Some("vendor-spike"));
        let meta = blocks.iter().find(|b| b.key == "__metadata").unwrap();
        assert_eq!(berry_field(&meta.lines, "cacheKey"), Some("10c0"));
        let lp = blocks.iter().find(|b| b.key == "\"left-pad@npm:1.3.0\"").unwrap();
        assert_eq!(berry_field(&lp.lines, "version"), Some("1.3.0"));
        assert_eq!(berry_field(&lp.lines, "resolution"), Some("left-pad@npm:1.3.0"));
