...
Socket's public patch proxy. (An API token unlocks your organization's patch tier; see
...
| `--api-token <token>` | `SOCKET_API_TOKEN` | Socket API token — create one in the [Socket dashboard](https://socket.dev) under your organization's API tokens settings. Use the raw token (`sktsec_<...>_api`) shown at generation time, **not** the `sha512-...` display hash. When omitted, the public patch proxy is used. |
...
- **[CLI contract](crates/socket-patch-cli/CLI_CONTRACT.md)** — the machine-readable
  ...
  governs them.
- **[Design notes](docs/design/)** — e.g. [why hosted mode is impossible for
  Go](docs/design/golang-hosted-no-go.md).
